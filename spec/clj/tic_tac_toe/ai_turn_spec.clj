  ...
  (context "next position"
    ...
    (it "ai move in the CLI"
      (let [expected (aic/ai-turn (board/get-board :3x3) "X" :hard)]
        (should= expected (gamec/next-position {:board (board/get-board :3x3) :ui :cli :players [:ai :ai]} ["X" :ai] :hard))))

    (it "ai vs ai move in the GUI"
      (with-redefs [sut/sleep (stub :sleep)]
        (let [expected (aic/ai-turn (board/get-board :3x3) "X" :hard)]
          (should= expected (gamec/next-position {:board (board/get-board :3x3) :ui :gui :players [:ai :ai]} ["X" :ai] :hard))
          (should-have-invoked :sleep))))

    (it "ai vs human move in the GUI"
      (with-redefs [sut/sleep (stub :sleep)]
       (let [expected (aic/ai-turn (board/get-board :3x3) "X" :hard)]
        (should= expected (gamec/next-position {:board (board/get-board :3x3) :ui :gui :players [:human :ai]} ["X" :ai] :hard))
